...
def get_screener_data():
    ...
    for s in stocks:
        ...
        try:
            ...
            # Hvis FMP afviser din API-nøgle, viser vi det i menuen!
            if 'Error Message' in r:
                st.sidebar.error(f"FMP Fejl: {r['Error Message']}")
                break
            ...
            if 'historical' in r:
                ...
                prev_avg = df['avg_vol'].iloc[-2] if len(df) > 1 and df['avg_vol'].iloc[-2] > 0 else 1
                
                rvol = round(last_day['volume'] / prev_avg, 2)
                
                if rvol >= 1.0: # Viser alt med RVOL over 1.0 i aften
                    results.append({"Symbol": s, "RVOL": rvol, "Vol": last_day['volume']})
        except Exception:
            continue
            
    if not results:
        return [{"Symbol": "NVDA", "RVOL": 0.0, "Vol": 0}]
...
# --- 4. DATA LOGIK (Den rettede version uden crash) ---
def fetch_data(symbol):
    try:
        # Vi tvinger den til at hente de sidste 5 dage, så vi rammer forbi weekenden
        end_date = datetime.now().strftime('%Y-%m-%d')
        start_date = (datetime.now() - timedelta(days=5)).strftime('%Y-%m-%d')
        
        # FEJLEN ER SLETTET HER: Ingen 'extended_hours=True' længere!
        m_bars = alpaca.get_bars(symbol, TimeFrame.Minute, start=start_date, end=end_date, limit=5000).df
        ...
        st.sidebar.error(f"Alpaca data-fejl: {e}")
...
with tab1:
    if not m_data.empty:
        # Tager kun de sidste 300 minutter af fredagens data for ikke at overbelaste skærmen
        ...
        st.error(f"Alpaca returnerede ingen historisk data for {ticker}. Tjek Daily fanen.")
